...
class PassportProcessing:
    ...
    """
    # endregion
    def __init__(self, raw_passport_feed):
        """
        docstring
        """
        self.raw_passport_feed = raw_passport_feed
        self.required_fields = ['byr', 'iyr', 'eyr', 'hgt', 'hcl', 'ecl', 'pid']
        self.optional_fields = ['cid']

    def parse_passport_feed(self, feed):
        passports = []
        passport = {}
    ...
    def count_valid_passports(self, passports):
        """
        docstring
        """
        valid_passports = 0
    ...
    def validate_passport(self, passport):
        ...
        if(all(item in passport.keys() for item in self.required_fields)):
            four_digits = r'\d{4}'
            ...
            valid = valid and re.match(four_digits, passport['byr']) and 1920 <= int(passport['byr']) <= 2002
            valid = valid and passport['ecl'] in ['amb', 'blu', 'brn', 'gry', 'grn', 'hzl', 'oth']
            valid = valid and re.match(four_digits, passport['eyr']) and 2020 <= int(passport['eyr']) <= 2030
            valid = valid and re.match(r'#[0-9a-f]{6}', passport['hcl'])
            valid = valid and re.match(four_digits, passport['iyr']) and 2010 <= int(passport['iyr']) <= 2020
            valid = valid and re.match(r'\d{9}', passport['pid'])
